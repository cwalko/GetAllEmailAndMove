...
Public Class GetEmailMove
    ...
    Public Sub _tMonitorOutLook()
        ...
        Catch
            ...
            GetMove.eMail_Setup(eMailContentFileName, "")  'Prepare email that this program is shutting down, most likely because Outlook is NOT running 
        End Try
        '  MultiBeep(356)
        Me.Show()
        Me.Refresh()
    ...
    Sub ProcessEmail()  'BtnProcessNew_Click(sender As Object, e As EventArgs) Handles BtnProcessNew.Click
        ...
        For i = olSourceFolder.Items.Count To 1 Step -1
            Try
                ...
                emailBody = mi.Body
                ' MsgBox("sqldb = " + SQLDB)
                Call GetMove.Insert_to_SQL(SQLDB, PayTransID, CreationTime, SentOn, ReceivedTime, SenderName, SenderEmailAddress, emailBody)
                ...
                If emailBodyLines(0) <> "Check if Credit Card Approved!!" Then ' if not a seafood order move it out of inbox and send an email
                    ...
                    GetMove.eMail_Setup(eMailContentFileName, "")  'Prepare to send email that an email has come in that is NOT an Ecommerse Order
                ...
                For Each s As String In emailBodyLines 'loop through each line of the email
                  ...
                  '     MsgBox("DeliveryType:" & DeliveryType)
                        Case 11  ' Requested Time of Dinner Pickup
                            ...
                            TimeOrderTaken = CreationTime.ToString("yyyy-MM-dd hh:mm")
                            ...
                            If Tdate.DayOfWeek <> DayOfWeek.Friday Then
                                NextFriday(Tdate)
                            End If
                            nFriday = Tdate.ToString("yyyy-MM-dd hh:mm")
                        ...
                        Case 15 'Approved Yes/No
                            ...
                            If Not Approved.IndexOf(":") = -1 Then
                                ...
                                Else
                                    ...
                                    GetMove.eMail_Setup(eMailContentFileName, PayTransID)  'Prepare email that an order has come in with a rejected CC 
                                    mi.Move(olBadCreditFolder)
                ...
                If Array.Find(IdFile, Function(x) (x.StartsWith(PayTransID))) <> "" Then ' Dupe email, already in list Id's we ahve processed in the IdFile
                    ...
                    ScrollBox("Text")
                    GetMove.eMail_Setup(eMailContentFileName, PayTransID)  'Prepare email that an order has come in that has already been processed, a Dupe 
                    mi.Move(olBadCreditFolder)
        ...
        + vbTab + OrderType + vbTab + DeliveryType + vbTab + PaymentType + vbTab + PaymentStatus + vbTab + TotalLine + vbTab + SenderEmailAddress
            ...
            Catch ex As NullReferenceException
                GetMove.eMail_Setup("System Down Unknown Error - restart system", "")  'We have found an error not recognized so we are shutting down and sending an email.  Send email warning someone the program has stopped

                MessageBox.Show("NullReferenceException: " & ex.Message)
                MessageBox.Show("Stack Trace: " & vbCrLf & ex.StackTrace)

                'If mi Is Nothing Then
                GetMove.eMail_Setup("System Down Bad Mail Object.txt", "")  'We have found an error not recognized so we are shutting down and sending an email.  Send email warning someone the program has stopped
                'Me.Close()
...
_Skip:
        ...
        TextBox1.Text = TextBox1.Text + "Starting Process to  Convert Text File to an Order!" + vbCrLf


        Dim sProcessName As String = "CreateOrdersFromTxtFiles.exe"
        ...
        Try
            ...
            Else
                ...
                Dim MyProgram As New System.Diagnostics.Process
                With MyProgram
                    .StartInfo.WorkingDirectory = "D:\Users\cwalk\source\repos\GetAllEmailAndMove\CreateOrdersFromTxtFiles\bin\Debug\"
                    .StartInfo.FileName = sProcessName
                End With
                MyProgram.Start()
            End If
        Catch ex As System.Exception
            MsgBox("System Exception = " + ex.ToString)
        End Try


        'Dim sProcessName As String = "CreateOrdersFromTxtFiles"
        'MsgBox("Create Orders is running = " + IsRunning(sProcessName).ToString)
        'If IsRunning(sProcessName) Then
        ...
        '        .StartInfo.FileName = sProcessName
        '    End With
        'MyProgram.Start() '**************** Commented out for testing ******************
        'End If
    ...
    End Sub
    Private Function IsRunning(sProcessName As String) As Boolean
        Dim bRet As Boolean = False
        Try
            Dim listProc() As System.Diagnostics.Process
            listProc = System.Diagnostics.Process.GetProcessesByName(sProcessName)
            If listProc.Length > 0 Then ' the program is running
                bRet = True
            End If
        Catch ex As System.Exception
        End Try
