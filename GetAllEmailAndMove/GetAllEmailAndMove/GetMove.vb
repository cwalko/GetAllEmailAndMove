...
Module GetMove
    Public PayTransID As String = GetEmailMove.PayTransID
    ' Public Property OrdersText As Object
    ...
    Public Sub Insert_to_SQL(SQLDB, PayTransID, CreationTime, SentOn, ReceivedTime, SenderName, SenderEmailAddress, emailBody)
        ...
        ' Dim Location As String = OrdersText.Where()
        ...
        Try
            MyConnection.Open()
            GetEmailMove.Label1.Text = MyConnection.State.ToString
            GetEmailMove.Label1.Refresh()
        ...
        Catch Err As Exception
            ...
            GetMove.eMail_Setup("ErrorFile", "")  'Send email warning someone the program has stopped
            ' SHUTTING DOWN THE BRIDGE SYTEM
            MyConnection.Close()
            MyConnection.Dispose()
            GetEmailMove.Dispose()
            MsgBox("SYSTEM HAS BEEN SHUT DOWN! - SQL CONNECTION WILL NOT OPEN on Line 16 of Module 1")
            System.Windows.Forms.Application.Exit()
            End ' End the program
        End Try
        If GetEmailMove.Label1.Text = "Open" Then
            Try
                ...
                Using reader As SqlDataReader = cmd1.ExecuteReader()
                    If reader.HasRows Then
                        ...
                        Call GetMove.WriteErrorFile(errorFile, PayTransID)  'Update the textbox with "IT IS ALREADY IN DATABASE" and error
                        GoTo Line65
...
Line65:
    ...
    Public Sub WriteErrorFile(errorFile, PayTransID)
        Try
            ...
            GetEmailMove.TextBox1.AppendText(errorFile)
            GetEmailMove.ScrollBox("Text")
            readFile.Close()
            readFile = Nothing
            Dim eMailContentFileName As String = errorFile
            GetMove.eMail_Setup(eMailContentFileName, PayTransID)  'Prepare email that this program is shutting down, most likely because Outlook is NOT running 
    ...
    Sub eMail_Setup(fileName As String, PayTransID As String)
        ...
        GetEmailMove.TextBox1.Text = GetEmailMove.TextBox1.Text + emailContent(1) + PayTransID + vbCrLf
        GetEmailMove.ScrollBox("Text")
